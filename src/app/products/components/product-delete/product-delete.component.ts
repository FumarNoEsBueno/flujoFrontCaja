      ...
      >
        <div class="p-6">
          ...
          <div class="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 class="text-lg font-bold text-surface-900">Eliminar Producto</h2>
          ...
          <div class="bg-surface-50 rounded-lg p-3.5 mb-5 border border-surface-100">
            <p class="text-sm text-surface-700">
              ...
              <span class="font-semibold text-surface-900">{{ producto().nombre }}</span>?
            </p>
          </div>
...
export class ProductDeleteComponent {
  ...
  producto  = input.required<ProductoTabla>();
  cerrar    = output<void>();
  eliminado = output<void>();

  mutation = injectMutation(() => ({
    mutationFn: (id: number) =>
      firstValueFrom(this.productoService.delete(id)),
    onSuccess: () => {
      this.queryClient.invalidateQueries({ queryKey: ['productos'] });
      this.toast.success('Producto eliminado', 'El producto fue eliminado correctamente.');
      this.eliminado.emit();
    },
    onError: () => {
      this.toast.error('No se pudo eliminar', 'Ocurrió un error al intentar eliminar el producto.');
    },
  }));

  confirmar(): void {
    this.mutation.mutate(this.producto().id);
  }
}
